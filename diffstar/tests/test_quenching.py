...
def test_quenching_function_has_expected_behavior_on_default_params():
    ...
    q = _quenching_kern(lgtarr, *DEFAULT_Q_PARAMS)
    ...
    actual_q_drop = _quenching_kern(lg_qt, lg_qt, q_dt, lg_q_drop, lg_q_rejuv)
    actual_lg_q_drop = np.log10(actual_q_drop)
...
def test_unbounded_and_bounded_quenching_functions_agree():
    ...
    q = _quenching_kern(lgtarr, *DEFAULT_Q_PARAMS)
    q2 = _quenching_kern_u_params(lgtarr, *DEFAULT_U_Q_PARAMS)
    assert np.allclose(q, q2, rtol=1e-3)
